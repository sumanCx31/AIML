...
export default function NepalHousePredictor() {
  ...
  const handlePredict = (e: any) => {
    ...
    const locationCoefficient =
      NEPAL_COEF[form.location as keyof typeof NEPAL_COEF];
    ...
    <div className="min-h-screen bg-linear-to-br from-teal-50 via-white to-emerald-50 flex items-center justify-center p-4">
      ...
      <div className="max-w-md w-full bg-white/80 backdrop-blur-xl border border-white/20 rounded-3xl shadow-2xl shadow-teal-500/10 p-8">
        <div className="flex items-center space-x-3 mb-6">
          ...
          <div>
            ...
            <p className="text-xs text-gray-500 font-medium">
              Kathmandu Valley Client-Side ML
            </p>
          </div>
        </div>
        ...
        <form onSubmit={handlePredict} className="space-y-4">
          <div className="grid grid-cols-2 gap-3">
            <div>
              ...
              <input
                ...
                required
                className="w-full px-3.5 py-2.5 bg-gray-50 border border-gray-200 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-teal-500 focus:bg-white transition"
              />
            </div>
            <div>
              <label className="block text-[11px] font-bold text-gray-600 uppercase tracking-wider mb-1">
          ...
          <button
            type="submit"
            className="w-full mt-2 py-3 bg-teal-600 hover:bg-teal-700 text-white font-semibold rounded-xl text-sm shadow-lg shadow-teal-600/25 transition-all flex items-center justify-center space-x-2 cursor-pointer"
          >
